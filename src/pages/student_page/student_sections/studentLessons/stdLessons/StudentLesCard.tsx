...
const StudentLesCard: React.FC<LesBoxProps> = ({
  ...
  ></iframe>  */}
      ...
      <p className={`${styles['student-lesBox_title']} overflow-hidden`}>{title}</p>
      <div className={styles.keys}>
        {keys.map((key, index) => (
          <div className={styles.studentkey} key={index}>
            {key}
          </div>
        ))}
      </div>
      <p className={styles['student-lesBox_description']}>{description}</p>
      <CustomBtn title="Մասնակցել" width />
      <CustomBtn title="Լքել դասընթացը" transparent width />
